...
function TransitionLoader({ duration = 5000, message = 'Loading...' }) {
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-[9999] flex items-center justify-center bg-gradient-to-br from-purple-50 via-white to-purple-50"
    >
      {/* Animated Background */}
      <div className="absolute inset-0">
        ...
        />
        <motion.div
          className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-violet-400/20 rounded-full blur-3xl"
          animate={{
            scale: [1.2, 1, 1.2],
            opacity: [0.5, 0.3, 0.5],
          }}
      ...
      <div className="relative text-center space-y-6">
        ...
        <motion.div className="flex justify-center">
          <div className="relative">
            ...
            {/* Icon Container */}
            <div className="relative bg-gradient-to-br from-purple-600 to-violet-600 p-6 rounded-2xl shadow-2xl">
              <motion.div
                animate={{
                  rotate: [0, 360],
                }}
...
export default function PageTransitionWrapper({ children, duration = 5000 }) {
  ...
  useEffect(() => {
    ...
    // Check if we're navigating FROM authenticator page TO dashboard/home
    // This ensures the book animation plays after successful authentication
    const isAuthToDashboardTransition = 
      previousPathRef.current !== null &&
      (
        (previousPathRef.current.includes('/authenticator-page') && location.pathname.includes('/Dashboard')) ||
        (previousPathRef.current.includes('/authenticator-page') && location.pathname.includes('/Home'))
      );
    ...
    const isLoginTabSwitch = 
      previousPathRef.current !== null &&
      (
        (previousPathRef.current.includes('/Vantalog/User/Log-In') && location.pathname.includes('/Vantalog/Admin/Log-In')) ||
        (previousPathRef.current.includes('/Vantalog/Admin/Log-In') && location.pathname.includes('/Vantalog/User/Log-In')) ||
        (previousPathRef.current.includes('/Vantalog/User/Sign-Up') && location.pathname.includes('/Vantalog/Admin/Sign-Up')) ||
        (previousPathRef.current.includes('/Vantalog/Admin/Sign-Up') && location.pathname.includes('/Vantalog/User/Sign-Up')) ||
        (previousPathRef.current.includes('/Vantalog/User/Log-In') && location.pathname.includes('/Vantalog/User/Sign-Up')) ||
        (previousPathRef.current.includes('/Vantalog/User/Sign-Up') && location.pathname.includes('/Vantalog/User/Log-In')) ||
        (previousPathRef.current.includes('/Vantalog/Admin/Log-In') && location.pathname.includes('/Vantalog/Admin/Sign-Up')) ||
        (previousPathRef.current.includes('/Vantalog/Admin/Sign-Up') && location.pathname.includes('/Vantalog/Admin/Log-In'))
      );
    ...
    // Force animation for authentication to dashboard transition (CRITICAL FIX)
